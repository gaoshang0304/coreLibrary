...
public class SPUtils {
     ...
     * 不需要外部创建实例 *
     */
    private SPUtils(@NonNull String fileName) {
     ...
     * @return SPManager
     */
    public static SPUtils get(@NonNull String fileName) {
    ...
    private synchronized SharedPreferences getPreferences() {
        if (null == mSP) {
            mSP = App.getInstance().getContext().getSharedPreferences(mFileName, Context.MODE_PRIVATE);
        }
        return mSP;
    }
